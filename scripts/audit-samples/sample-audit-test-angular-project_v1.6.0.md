...
- `src/app/components/user-list/user-list.component.ts:49`
  ```typescript
  this.http.get<User[]>('https://api.example.com/users').subscribe(
  ```
- `src/app/components/user-list/user-list.component.ts:64`
  ```typescript
  this.http.get('https://api.example.com/config').subscribe((config) => {
  ```
- `src/app/components/user-list/user-list.component.ts:78`
  ```typescript
  this.http.delete(`https://api.example.com/users/${userId}`).subscribe(() => {
  ```
- `src/app/components/user-list/user-list.component.ts:86`
  ```typescript
...
### JS002 — addEventListener sans removeEventListener
...
**Correction :** 1) Preferer `@HostListener('event') method() {}` qui auto-cleanup avec le composant. 2) Pour des elements arbitraires, utiliser `Renderer2.listen(target, event, handler)` qui retourne une fonction de cleanup a stocker et appeler dans `ngOnDestroy()`. 3) Si `addEventListener` brut indispensable (ex: Worker, SSE), garder la reference du handler et appeler `removeEventListener(event, handler)` dans `ngOnDestroy`.
...
- `src/app/components/user-list/user-list.component.ts:43`
  ...
  ```
- `src/app/components/user-list/user-list.component.ts:46`
  ```typescript
...
- `src/app/components/user-list/user-list.component.html:10`
  ...
  ```
- `src/app/components/user-list/user-list.component.html:30`
  ```typescript
...
- `src/app/components/user-list/user-list.component.ts:43`
  ...
  document.addEventListener('keydown', (e: any) => {
  ```
- `src/app/components/user-list/user-list.component.ts:71`
  ```typescript
  this.filteredUsers = this.users.filter((u: any) =>
  ```
- `src/app/components/user-list/user-list.component.ts:76`
  ```typescript
  deleteUser(userId: any): void {  // TYPE001: any
  ```
- `src/app/components/user-list/user-list.component.ts:79`
  ```typescript
...
- `src/app/components/user-list/user-list.component.ts:91`
  ```typescript
  const data = raw as any;
  ```
- `src/app/components/user-list/user-list.component.ts:101`
  ```typescript
...
- `src/app/components/user-list/user-list.component.html:7`
  ...
  ```
- `src/app/components/user-list/user-list.component.html:28`
  ```typescript
  <img [src]="user.avatar">
  ```
...
- `src/app/components/user-list/user-list.component.html:19`
  ...
  ```
- `src/app/components/user-list/user-list.component.html:33`
  ```typescript
  <span class="action-link" (click)="viewProfile(user.id)">voir profil</span>
  ```

### A11Y003 — Anchor <a> sans href
...
**Correction :** Soit ajouter un `href` reel : `<a [routerLink]="['/path']">` pour le router Angular, ou `<a [href]="url">` pour un lien externe. Soit utiliser un vrai `<button>` avec un style 'lien' via CSS (souligne, couleur primaire). Pour navigation = anchor avec href ; pour action = button.

**Occurrences (2) :**

- `src/app/components/user-list/user-list.component.html:22`
  ```typescript
  <a class="link-style" (click)="goHome()">Retour accueil</a>
  ```
- `src/app/components/user-list/user-list.component.html:23`
  ```typescript
  <a (click)="exportCsv()">Exporter en CSV</a>
  ```
...
- `src/app/components/user-list/user-list.component.ts:46`
  ...
  window.addEventListener('scroll', () => console.log('scroll'));
  ```
- `src/app/components/user-list/user-list.component.ts:54`
  ```typescript
  console.log('Users loaded:', data.length);  // DEBUG001: encore un console.log
  ```
- `src/app/components/user-list/user-list.component.ts:57`
  ```typescript
...
- **addEventListener sans removeEventListener** (JS002) — Un `addEventListener` (sur document, window ou un element natif via ElementRef) ...
- **Anchor <a> sans href** (A11Y003) — Un `<a>` avec `(click)` mais sans `href` (ni `[href]`, ni `[routerLink]`) est un...
...
*Rapport genere par Angular Code Audit v1.5.0 — 2026-05-06 12:29*  
